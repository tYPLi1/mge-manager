...
export default function PlayerDetail() {
  ...
  if (!playerId) return <div className="text-gray-400 p-6">{t("playerDetail.noPlayerSelected")}</div>;
  if (isLoading) return <div className="text-gray-400 p-6 text-center">{t("playerDetail.loading")}</div>;

  const player = players.find(p => p.id === playerId);
  if (!player) return <div className="text-gray-400 p-6">{t("playerDetail.playerNotFound")}</div>;
    ...
    <div>
      <div className="mb-4">
        <Link to={createPageUrl("Leaderboard")} className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-300 transition-colors">
          <ArrowLeft className="w-3.5 h-3.5" /> {t("playerDetail.backToLeaderboard")}
        </Link>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div className="bg-[#111827] rounded-xl border border-white/5 p-4">
          <p className="text-xs text-gray-400 mb-1">{t("playerDetail.currentDkp")}</p>
          <DKPValue value={currentDkp} size="lg" />
        </div>
        <div className="bg-[#111827] rounded-xl border border-white/5 p-4">
          <p className="text-xs text-gray-400 mb-1">{t("playerDetail.totalEarned")}</p>
          <p className="text-lg font-bold font-mono text-white">{(player.total_dkp || 0).toLocaleString()}</p>
        </div>
        <div className="bg-[#111827] rounded-xl border border-white/5 p-4">
          <p className="text-xs text-gray-400 mb-1">{t("playerDetail.dkpSpent")}</p>
          <p className="text-lg font-bold font-mono text-gray-400">{Math.abs(player.dkp_spent || 0).toLocaleString()}</p>
        </div>
        <div className="bg-[#111827] rounded-xl border border-white/5 p-4">
          <p className="text-xs text-gray-400 mb-1">{t("playerDetail.status")}</p>
          <StatusBadge cooldownUntil={player.cooldown_until} />
          {player.cooldown_until && new Date(player.cooldown_until) > new Date() && (
            <p className="text-xs text-gray-500 mt-1">{t("playerDetail.cooldownUntil", { date: player.cooldown_until })}</p>
          )}
        </div>
      </div>
        ...
        <div className="bg-[#111827] rounded-xl border border-white/5 p-4">
          ...
          {powerRank > 0 && (
            <span className={`inline-flex mt-1 px-2 py-0.5 rounded text-xs font-medium ${isTop20 ? "bg-amber-500/15 text-amber-400" : "bg-gray-500/15 text-gray-400"}`}>
              {isTop20 ? t("playerDetail.top20", { rank: powerRank }) : t("playerDetail.outsideTop20", { rank: powerRank })}
            </span>
          )}
        </div>
        <div className="bg-[#111827] rounded-xl border border-white/5 p-4">
        ...
        <div className="max-h-72 overflow-y-auto">
          <table className="w-full">
            <thead className="bg-[#0d1117] sticky top-0">
              <tr>
                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.date")}</th>
                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.source")}</th>
                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase hidden sm:table-cell">{t("playerDetail.cols.type")}</th>
                <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.dkp")}</th>
              </tr>
            </thead>
          ...
          {transactions.length === 0 && <p className="text-center text-gray-500 text-sm py-6">{t("playerDetail.noTransactions")}</p>}
        </div>
      </div>
      ...
      {auctionResults.length > 0 && (
        <>
          <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">{t("playerDetail.auctionHistory")}</h3>
          <div className="bg-[#111827] rounded-xl border border-white/5 overflow-hidden mb-6">
            <table className="w-full">
              <thead className="bg-[#0d1117]">
                <tr>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.rank")}</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.dkpBid")}</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.targetScore")}</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.medals")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
      ...
      {penalties.length > 0 && (
        <>
          <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-3">{t("playerDetail.penaltyHistory")}</h3>
          <div className="bg-[#111827] rounded-xl border border-white/5 overflow-hidden">
            <table className="w-full">
              <thead className="bg-[#0d1117]">
                <tr>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.date")}</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.level")}</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.offenseCount")}</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.dkp")}</th>
                  <th className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase">{t("playerDetail.cols.status")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {penalties.map(p => (
